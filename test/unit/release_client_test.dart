...
void main() {
  ...
  group('ReleaseClient', () {
    test('Fetch release details', () async {
      final response = await client.releases.releases(
        249504,
      ); // Example release ID
    ...
    test('Fetch release rating by user', () async {
      final response = await client.releases.releasesRatingByUser(
        249504,
        username: 'bartman',
      ); // Example release ID and username
      expect(response, isA<Map<String, dynamic>>());
    ...
    test('Fetch release rating', () async {
      final response = await client.releases.releasesRating(
        249504,
      ); // Example release ID
      expect(response, isA<Map<String, dynamic>>());
    ...
    test('Fetch release stats', () async {
      final response = await client.releases.releasesStats(
        249504,
      ); // Example release ID
